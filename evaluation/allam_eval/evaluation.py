...
def load_dialect_clf(device: str):
    ...
    model= AutoModelForSequenceClassification.from_pretrained(DID_ID).to(device).eval()
    ...
    for raw, idx in label2id.items():
        if "MSA" in raw:
            msa_idx = idx
            break
    return tok, model, id2label, label2id, msa_idx

def camel_probs_batched(texts: List[str], tok, model, device: str, bs: int = BATCH_SIZE_CLF):
    all_probs = []
...
def evaluate_pred_files(pred_files: List[Path], device: str, out_dir: Path = OUT_DIR) -> list[list]:
    tok, clf, id2label, label2id, msa_idx = load_dialect_clf(device)
    summary = []
    ...
    for pf in pred_files:
        ...
        probs = camel_probs_batched(preds, tok, clf, device=device)
...
def save_summary_csv(summary: list[list], out_dir: Path = OUT_DIR, filename: str = "summary_saudi_only.csv") -> Path:
